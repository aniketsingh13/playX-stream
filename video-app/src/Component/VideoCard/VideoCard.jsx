...
import AddToHistory from "../../Service/HistoryService/AddToHistory";

const VideoCard = ({ videos }) => {
  const { _id, thumbnail, duration, title, avatar, alt, creatorName, views } =
    videos;
  const { featureDispatch } = useFeature();

  const historyHandler = () => {
    AddToHistory(videos, featureDispatch);
  };

  return (
    <Link to={`/videolisting/${_id}`} onClick={historyHandler}>
